...
if (isset($_POST['submit'])) {
	...
	if (isset($_GET['editid'])) {
		$query = "UPDATE treatment_records SET appointmentid='$_POST[select2]',treatmentid='$_POST[select4]',patientid='$_POST[patientid]',doctorid='$_POST[select5]',treatment_description='$_POST[textarea]',treatment_date='$_POST[treatmentdate]',treatment_time='$_POST[treatmenttime]',status='Active' WHERE appointmentid='$_GET[editid]'";
		...
		$query = "INSERT INTO treatment_records(appointmentid,treatmentid,patientid,doctorid,treatment_description,treatment_date,treatment_time,status) values('$_POST[select2]','$_POST[select4]','$_POST[patientid]','$_POST[select5]','$_POST[textarea]','$_POST[treatmentdate]','$_POST[treatmenttime]','Active')";
										...
										<form action="" method="POST">
											<div class="position-relative form-group">
												...
												?>
													...
													<?php
													$querynurse = "SELECT * FROM nurse_tbl INNER JOIN department_tbl ON department_tbl.id=nurse_tbl.departmentid WHERE nurse_tbl.id='$_SESSION[id]'";
													$executenurse = mysqli_query($db_connect, $querynurse);
													while ($rsnurse = mysqli_fetch_array($executenurse)) {
														if ($rsnurse['id']) {
															echo "<option value='$rsnurse[id]' selected disabled>$rsnurse[nurseLastname] ( $rsnurse[departmentName] ) </option>";
														} else {
															echo "<option value='$rsnurse[id]'>$rsnurse[nurseLastname] ( $rsnurse[departmentName] )</option>";
														}
													}
													?>
										...
										<table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
											...
											<tbody>
												...
												while ($rs = mysqli_fetch_array($execute)) {
													...
													$querynurse = "SELECT * FROM nurse_tbl WHERE id='$_SESSION[id]'";
													$executenurse = mysqli_query($db_connect, $querynurse);
